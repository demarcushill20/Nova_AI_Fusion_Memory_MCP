"""PLAN-0759 Phase 5b — retroactive backfill for PROMOTED_FROM edges.

Context
-------

Phase 5b adds a write-time promotion hook to
...
script lands**) promotes a memory from a lower layer (e.g. ``episodic``)
to a higher layer (e.g. ``semantic``), it passes
``metadata["_promoted_from"] = {"old_id", "from_layer", "to_layer"}`` so
the hook can MERGE a ``(new:base)-[:PROMOTED_FROM]->(old:base)`` edge.

This script is the **retroactive rail**: it scans the live Neo4j graph
for ``:base`` nodes that were promoted *before* the write-time hook was
wired (detectable via a historical ``promoted_from_id`` or
``_promoted_from`` property on the node) and retro-emits the
``PROMOTED_FROM`` edge via the same
:class:`MemoryEdgeService.on_memory_promote` codepath as the write-time
hook.

**Current state of the graph (as of Phase 5b landing):** promotion data
does NOT yet flow into Fusion Memory — ``nova-core/agents/`` promotion
callers have not been wired to pass the ``_promoted_from`` metadata, so
no historical promotion markers exist on ``:base`` nodes. A dry-run of
...
shape now so that the day Phase 5b's cross-repo wiring lands (the
separate B3 follow-up), the backfill rail is ready without another
engineering pass.

Sources of truth (do not duplicate logic — call these directly)
---------------------------------------------------------------

- :meth:`MemoryEdgeService.on_memory_promote` — the canonical promotion
  edge writer. Enforces whitelist via ``build_merge_edge_cypher``,
  stamps ``edge_version``, ``created_by="edge_service.on_memory_promote"``,
  and attaches ``{"from_layer", "to_layer"}`` metadata.
- :data:`memory_edges.EDGE_VERSION` — schema version stamp (inherited
  through ``on_memory_promote`` → ``MemoryEdge``).

Usage
-----
Dry-run (safe, writes nothing)::

    python -m scripts.assoc_backfill_provenance_promotion \\
        --run-id phase5b-promote-2026-04-21 --dry-run --verbose

Full live run (writes edges)::

    python -m scripts.assoc_backfill_provenance_promotion \\
        --run-id phase5b-promote-2026-04-21 --rate-limit-qps 5.0

Rollback::

    python -m scripts.assoc_rollback \\
        --run-id backfill-phase5b-promote-2026-04-21
"""
...
LOGGER = logging.getLogger("assoc_backfill_provenance_promotion")
...
class BackfillReport:
    """Structured summary of the promotion backfill run."""
    ...
    for prefix in (
        ...
        if stripped.startswith(prefix):
            raise BackfillError(
                ...
                "prefix (e.g. 'phase5b-promote-YYYYMMDD')."
            )
    if not allow_test:
...
# ---------------------------------------------------------------------------
# Candidate scan — searches for historical promotion markers
# ---------------------------------------------------------------------------

#: Cypher that finds ``:base`` nodes with a historical promotion marker.
#:
#: Two property names are checked because the upstream wiring is not
#: finalized yet:
#:   - ``n.promoted_from_id`` (scalar property naming)
#:   - ``n._promoted_from`` (matches the metadata key that the write-time
#:     hook consumes — kept here in case a legacy writer persisted it as
#:     a flat node property rather than a metadata-only signal).
#:
#: The script returns whichever marker is present and, for compatibility
#: with a future layered property, optionally ``n.promoted_from_layer`` /
#: ``n.promoted_to_layer``. On the current graph this query returns 0 rows.
_PROMOTION_CANDIDATES_CYPHER: str = (
    "MATCH (n:base) "
    "WHERE n.promoted_from_id IS NOT NULL "
    "   OR n._promoted_from IS NOT NULL "
    "RETURN n.entity_id AS new_id, "
    "       coalesce(n.promoted_from_id, n._promoted_from) AS old_id, "
    "       n.promoted_from_layer AS from_layer, "
    "       n.promoted_to_layer AS to_layer "
    "ORDER BY n.entity_id ASC"
)


async def _iter_promotion_candidates(
    driver: "AsyncDriver",
    *,
    database: str,
) -> "AsyncIterator[Dict[str, Any]]":
    """Async generator yielding promotion candidate rows.

    Each row is a dict with keys ``new_id``, ``old_id``, ``from_layer``,
    ``to_layer``. ``from_layer`` / ``to_layer`` may be ``None`` if the
    historical writer did not record them — the caller decides whether
    to skip or fall back to placeholder layer strings.
    """
    async with driver.session(database=database) as session:
        result = await session.run(_PROMOTION_CANDIDATES_CYPHER)
        async for rec in result:
            yield {
                "new_id": rec["new_id"],
                "old_id": rec["old_id"],
                "from_layer": rec["from_layer"],
                "to_layer": rec["to_layer"],
            }
...
async def backfill_promotion_edges(
    *,
    run_id: str,
    driver: "AsyncDriver",
    database: str = "neo4j",
    ...
    """Scan historical promotion markers and retro-emit ``PROMOTED_FROM`` edges.
    ...
    try:
        async for row in _iter_promotion_candidates(
            ...
            if not new_id or not old_id:
                ...
                LOGGER.debug(
                    "backfill.skip_missing_ids new_id=%r old_id=%r",
                    new_id,
                    old_id,
                )
                continue

            # ``on_memory_promote`` requires both layers or neither. If the
            # historical writer captured only one of the two, drop the pair
            # into the skip bucket rather than fabricate the other half.
            if (from_layer is None) != (to_layer is None):
                report.candidates_skipped_missing_fields += 1
                LOGGER.debug(
                    "backfill.skip_half_pair_layers new_id=%r from_layer=%r to_layer=%r",
                    new_id,
                    from_layer,
                    to_layer,
                )
            ...
            if dry_run:
                ...
                processed_counter += 1
                LOGGER.debug(
                    "backfill.dry_run candidate new_id=%s old_id=%s "
                    "from_layer=%s to_layer=%s",
                    new_id,
                    old_id,
                    from_layer,
                    to_layer,
            ...
            try:
                await edge_service.on_memory_promote(
                    new_id=new_id,
                    old_id=old_id,
                ...
                LOGGER.warning(
                    "backfill.upsert_failed new_id=%s old_id=%s error=%s",
                    new_id,
                    old_id,
                    exc,
                )
...
def _parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="assoc_backfill_provenance_promotion",
        description=(
            "PLAN-0759 Phase 5b — retroactively create PROMOTED_FROM "
            "edges for any :base memory carrying a historical promotion "
            "marker. On the current graph this is expected to find 0 "
            "candidates; the script is the backfill rail, ready for when "
            "Phase 5b's cross-repo wiring lands."
        ),
    )
    parser.add_argument(
        "--run-id",
...
async def _async_main(args: argparse.Namespace) -> int:
    ...
    try:
        report = await backfill_promotion_edges(
            run_id=args.run_id,
            driver=driver,
    ...
    if report.candidates_scanned == 0:
        print(
            "0 candidates found for retroactive promotion backfill — "
            "nothing to do. Promotion data does not yet flow into Fusion "
        ...
        print(
            f"DRY RUN — would create {report.edges_created} PROMOTED_FROM "

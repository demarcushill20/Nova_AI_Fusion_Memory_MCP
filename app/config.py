...
class Settings(BaseSettings):
    ...
    # ------------------------------------------------------------------ #
    ASSOC_SIMILARITY_WRITE_ENABLED: bool = True  # PLAN-0759 Phase 1: similarity_linker writes (:base)-[:SIMILAR_TO]->(:base). Flipped True 2026-04-23 Sprint 21 — backfill produced 1,221 SIMILAR_TO edges with zero failures; write-time linker has 14 tests green; hub-suppression + rate limiting already in place; semaphore-bounded (MAX_IN_FLIGHT=32) fire-and-forget can't bubble failures to perform_upsert.
    ASSOC_ENTITY_WRITE_ENABLED: bool = True  # PLAN-0759 Phase 2: entity_linker writes MENTIONS edges to canonical entities. Flipped True 2026-04-23 Sprint 21 — backfill produced 9,081 MENTIONS edges across 2,837 entity nodes; hub-suppression at >50 and per-memory cap MAX_ENTITIES_PER_MEMORY=20 enforced; fire-and-forget semaphore-bounded.
    ASSOC_TEMPORAL_WRITE_ENABLED: bool = True  # PLAN-0759 Phase 3: temporal_linker writes MEMORY_FOLLOWS adjacency edges within a session. Flipped True 2026-04-23 Sprint 21 — backfill produced 182 edges across 85 multi-member sessions; linker skips sessionless memories; fire-and-forget semaphore-bounded.
    ...
    ASSOC_COOCCURRENCE_WRITE_ENABLED: bool = True  # PLAN-0759 Phase 7a: cooccurrence linker writes CO_OCCURS edges between memories sharing ≥2 non-hub entities. Flipped True 2026-04-23 Sprint 21 — backfill produced 4,734 CO_OCCURS edges (canonical dedup); hub-suppression at >50 mention count, per-entity degree cap of 30, IDF-normalized weight floor 0.01. Requires ASSOC_ENTITY_WRITE_ENABLED to produce non-trivial edge counts on new writes.
    ASSOC_TASK_HEURISTIC_WRITE_ENABLED: bool = True  # PLAN-0759 Phase 7b: task_heuristic linker. Flipped True 2026-04-23 after Phase 6 parity gate cleared (precondition per phase6_gate_result.md §952) and all 14 tests in test_task_heuristic_linker.py green. Hook only fires when a memory lands with `category=="bug_fix"` AND `metadata["fixes_task_id"]` points at a same-project `task_failed` memory within 30 days — no-op for callers that don't opt in, so safe default-on.

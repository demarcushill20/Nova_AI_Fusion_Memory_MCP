...
routing modes. No external dependencies needed.
"""
...
class TestNonTemporalRouting:
    ...
    def test_relationship_query(self, router):
        """'what is' (vector) + 'relationship' (graph) → HYBRID."""
    ...
    def test_both_vector_and_graph(self, router):
        """Both vector and graph keywords → HYBRID."""
...
class TestRouterEdgeCases:
    ...
    def test_temporal_keyword_substring(self, router):
        """'last' appears in 'blast' — should still match due to substring."""
        mode = router.route("blast radius of changes")
        # 'last' is a substring of 'blast', so this will match temporal

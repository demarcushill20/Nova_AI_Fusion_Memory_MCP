...
try:
    ...
    pytestmark = pytest.mark.skip(
        reason="Manual MCP integration harness; run directly with `python test_mcp_adapter.py`."
    )
except Exception:
    pytest = None
...
def run_mcp_command(command, args=None):
    ...
    request = {"tool": command, "args": args}
    ...
    cmd = ["docker", "run", "-i", "--rm", "--network=host", "nova-memory-mcp:latest"]
...
def test_upsert_memory():
    ...
    # Create metadata
    metadata = {"source": "test_script", "timestamp": time.time(), "test": True}

    # Call the upsert tool
    response = run_mcp_command(
        "upsert_memory",
        {"content": memory_content, "id": memory_id, "metadata": metadata},
    )
...
def test_query_memory(query_text):
    ...
    response = run_mcp_command("query_memory", {"query": query_text})
...
def test_delete_memory(memory_id):
    ...
    response = run_mcp_command("delete_memory", {"memory_id": memory_id})
